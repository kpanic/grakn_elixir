defmodule Grakn do
  ...
  @spec query!(conn(), [%Grakn.Query{}], Keyword.t()) :: any()
  def query!(conn, queries, opts \\ [])

  def query!(conn, queries, opts) when is_list(queries) do
    DBConnection.execute!(conn, queries, [], with_transaction_config(opts))
  end
